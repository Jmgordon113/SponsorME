...
function OpportunityFeed() {
  ...
  const filtered = opportunities.filter((opp) => {
    ...
    if (search && !opp.title.toLowerCase().includes(search.toLowerCase())) pass = false;
    if (minAmount) {
          ...
          {filtered.map((opp) => (
            <Link to={`/opportunity/${opp._id}`} key={opp._id}>
              <div className="opp-card">
                ...
                <h3>{opp.title}</h3>
                {opp.tagline && <p className="category-tag">{opp.tagline}</p>}
                <p className="category-tag">{opp.category}</p>
                <p className="description">
                  {opp.description && opp.description.length > 100
